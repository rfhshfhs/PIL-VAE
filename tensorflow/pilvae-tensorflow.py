...
def ppcamle(data, q):
    ...
    print("D:", D)
    sorted_indices = np.argsort(D)[::-1]  # 对于D的排序，确保它们按降序排序，以确保最大的特征值在前面
...
# 输出数据集的形状
print("合并后的数据集形状:", train_images.shape, train_images.shape)
# 合并训练集和测试集
all_images = tf.concat([train_images, test_images], axis=0)
all_labels = tf.concat([train_labels, test_labels], axis=0)
# 输出合并后的数据集的形状
